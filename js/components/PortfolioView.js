...
export class PortfolioView {
    ...
    introBox.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 280px;">
          ...
          <p>Organize your completed freelance work into polished case studies. Draft customer testimonials, record measurable results, and update showcase status options.</p>
        </div>
        <button class="btn btn-primary" id="btn-portfolio-add-work" style="gap: 6px;">
          ${getIcon('plus', '', 16)} Add Completed Project
        </button>
      </div>
    `;
  ...
  renderGridOnly() {
    ...
    if (showcased.length === 0) {
      canvas.innerHTML = `
        <div style="grid-column: 1 / -1;" class="empty-state-box">
          ${getIcon('folder', '', 48)}
          <h3>No portfolio studies planned</h3>
          <p>Showcase completed freelance projects to demonstrate visual outcomes and attract clients.</p>
          <button class="btn btn-secondary" id="btn-empty-portfolio-add">${getIcon('plus', '', 14)} Add Completed Project</button>
        </div>
      `;
    ...
    showcased.forEach(p => {
      ...
      card.innerHTML = `
        <div class="portfolio-cover-mock">
          ...
          <div class="portfolio-cover-overlay" style="display: flex; flex-direction: column; align-items: flex-end; gap: 6px;">
            ...
            <span class="client-status-badge ${visibilityClass}" style="font-size: 0.65rem; padding: 2px 8px; backdrop-filter: blur(8px); background: rgba(0,0,0,0.5);">${p.showcaseStatus || 'Draft Case Study'}</span>
          </div>
        </div>
        <div class="portfolio-card-content" style="gap: 12px;">
          <div>
            ...
            <span class="stat-subtext" style="font-size: 0.72rem;">Client: ${p.clientName}</span>
          </div>
          
          <div>
            <span class="drawer-meta-title" style="font-size: 0.65rem;">Short Case Summary</span>
            <p class="portfolio-case-desc" style="font-size: 0.78rem;">
              ${p.portfolioDescription || '<em>No case study summary written yet. Click edit below to draft your outcomes.</em>'}
            </p>
          </div>
          ...
          ${p.resultDescription ? `
            <div style="background: rgba(255,255,255,0.02); padding: 8px 10px; border-left: 2px solid var(--color-secondary); border-radius: 0 4px 4px 0;">
              <span class="drawer-meta-title" style="font-size: 0.65rem; color: var(--color-secondary);">Measurable Result</span>
          ...
          ${p.testimonial ? `
            <div style="background: rgba(255,255,255,0.01); padding: 8px 10px; border-radius: 4px; border: 1px dashed var(--border-subtle);">
              <span class="drawer-meta-title" style="font-size: 0.65rem; color: var(--color-accent);">Client Testimonial</span>
          ...
          <div class="portfolio-metadata-row" style="padding-top: 10px;">
            <button class="invoice-btn-small edit-case-trigger" style="display: flex; align-items: center; justify-content: center; gap: 4px; padding: 6px;">
              ${getIcon('edit', '', 12)} Edit Case Study
            </button>
      ...
      card.querySelector('.remove-trigger').addEventListener('click', () => {
        if (confirm(`Remove "${p.title}" from showcase index?`)) {
          this.store.updateProject(p.id, { portfolioShowcase: false });
          this.onTriggerToast('Project removed from portfolio');
          this.render();
        }
      });
  ...
  showEditCaseStudyModal(project) {
    ...
    modalOverlay.innerHTML = `
      <div class="modal-container" style="max-width: 560px; max-height: 85vh;">
        <div class="modal-header">
          <h3>Edit Portfolio Case Study</h3>
        ...
        <div class="modal-body" style="display: flex; flex-direction: column; gap: 14px; padding: 16px 24px;">
          ...
          <div class="form-group">
            <label for="case-status-select">Showcase Status</label>
            <select id="case-status-select" class="form-control">
              <option value="Private" ${statusVal === 'Private' ? 'selected' : ''}>Private (Internal Staging Only)</option>
              <option value="Draft Case Study" ${statusVal === 'Draft Case Study' ? 'selected' : ''}>Draft Case Study</option>
              <option value="Ready to Showcase" ${statusVal === 'Ready to Showcase' ? 'selected' : ''}>Ready to Showcase</option>
              <option value="Published" ${statusVal === 'Published' ? 'selected' : ''}>Published Live</option>
            </select>
          </div>

          <div class="form-group">
            <label for="case-img-url">Mockup Cover Image URL</label>
            <input type="url" id="case-img-url" class="form-control" value="${imgVal}" placeholder="Paste mockup image URL...">
          </div>

          <div class="form-group">
            <label for="case-summary-text">Short Case Study Summary</label>
            <textarea id="case-summary-text" class="form-control" style="min-height: 80px;" placeholder="Summarize project goals, scope...">${descVal}</textarea>
          </div>

          <div class="form-group">
            <label for="case-result-text">Result Description</label>
            <textarea id="case-result-text" class="form-control" style="min-height: 60px;" placeholder="Specific result details e.g., Boosted clickthroughs by 14%...">${resultVal}</textarea>
          </div>

          <div class="form-group">
            <label for="case-testimonial-text">Client Testimonial</label>
            <textarea id="case-testimonial-text" class="form-control" style="min-height: 60px;" placeholder="What did the client say about your delivery?">${testVal}</textarea>
          </div>
    ...
    modalOverlay.querySelector('#save-case-modal').addEventListener('click', () => {
      ...
      this.onTriggerToast('Portfolio case study saved', 'text-success');
      closeActions();
      this.render();
    });
  }
  ...
  showAddCompletedProjectDrawer() {
      ...
      drawerOverlay.innerHTML = `
        <div class="drawer-panel">
          ...
          <div class="drawer-body" style="display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; gap: 16px; padding: 48px 24px;">
            ${getIcon('alert', '', 40)}
            <h4 style="color: var(--text-primary);">No completed projects to add</h4>
            <p style="font-size: 0.85rem; color: var(--text-secondary); line-height: 1.4;">
              Only projects in the <strong>Completed</strong> column can be staged as case studies. Complete active work on your board first!
            </p>
            <button class="btn btn-secondary" id="cancel-p-drawer-empty" style="margin-top: 10px;">Got it</button>
          </div>
        </div>
      `;
      ...
      <div class="drawer-panel">
        ...
        <div class="drawer-body">
          <form id="portfolio-add-form">
            ...
            <div class="form-group">
              <label for="p-select-choice">Select Completed Work</label>
            ...
            <div class="form-group">
              <label for="p-show-status">Showcase Status</label>
              <select id="p-show-status" class="form-control">
                <option value="Draft Case Study" selected>Draft Case Study</option>
                <option value="Ready to Showcase">Ready to Showcase</option>
                <option value="Published">Published</option>
              </select>
            </div>

            <div class="form-group">
              <label for="p-summary">Short Case Study Summary</label>
              <textarea id="p-summary" class="form-control" style="min-height: 80px;" placeholder="Summarize project scope, what you built..."></textarea>
            </div>

            <div class="form-group">
              <label for="p-result">Result Description</label>
              <textarea id="p-result" class="form-control" style="min-height: 60px;" placeholder="Specific result details e.g., Boosted clickthroughs by 14%..."></textarea>
            </div>

            <div class="form-group">
              <label for="p-testimonial">Client Testimonial</label>
              <textarea id="p-testimonial" class="form-control" style="min-height: 60px;" placeholder="What did the client say?"></textarea>
            </div>

            <div class="modal-footer" style="padding: 16px 0 0 0; border: none;">
              <button type="button" class="btn btn-secondary" id="cancel-p-drawer">Cancel</button>
              <button type="submit" class="btn btn-primary">Stage Case Study</button>
            </div>
    ...
    form.addEventListener('submit', (e) => {
      ...
      this.onTriggerToast('Project added to portfolio showcase', 'text-success');
      closeActions();
      this.render();
    });
  }
